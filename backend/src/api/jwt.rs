...
use axum::{extract::{Extension, State}, http::{header::SET_COOKIE, Response, StatusCode}, response::IntoResponse, routing::post, Json, Router};
...
pub fn routes(state: Arc<AppState>) -> Router{
    let jwt_routes = Router::new()
        ...
        .with_state(state).with_state(CookieJar::default());
...
async fn refresh_jwt(state: State<Arc<AppState>>, cookie_jar:CookieJar, req: Json<RefreshRequest>) -> impl IntoResponse{
    ...
    let refresh_token_claims = crate::utils::jwt::verify_jwt(&refresh_token)
    ...
    let new_jwt = crate::utils::jwt::create_jwt(&new_jwt_claims);
